...
export async function POST(request: NextRequest) {
  ...
  const { url, extracted_text, retriever_domains, gueltig_von, gueltig_bis } =
    await request.json();

  if (!url || !extracted_text || !retriever_domains?.length) {
...
Forme den folgenden Webseiteninhalt in einen kompakten Wissens-Eintrag für den Retriever "${domain}" um.
...
{"page_content":"...","metadata":{"titel":"...","kategorie":"...","tags":["..."],"fragevarianten":["...","...","..."]}}`,
      ...
      try {
        ...
        parsed = { page_content: extracted_text.slice(0, 500), metadata: { titel: url } };
      }
      ...
      console.error(`[Admin] ingest-url failed for domain ${domain}:`, err);
      errors.push(domain);
    }
  }
