...
export async function POST(request: NextRequest) {
  ...
  const { freitext, retriever_domains, gueltig_von, gueltig_bis } = await request.json();

  if (!freitext || !retriever_domains?.length) {
...
Forme den folgenden Freitext in einen kompakten Wissens-Eintrag für den Retriever "${domain}" um.
...
{"page_content":"...","metadata":{"titel":"...","kategorie":"...","tags":["..."],"fragevarianten":["...","...","..."]}}`,
      ...
      try {
        ...
        console.error("[Admin] ingest: LLM returned non-JSON, using raw freitext");
        parsed = {
          page_content: freitext,
          metadata: { titel: freitext.slice(0, 80) },
        };
      }
      ...
      console.error(`[Admin] ingest failed for domain ${domain}:`, err);
      errors.push(domain);
    }
  }

  if (inserted === 0) {
    return NextResponse.json({ success: false, error: "Kein Eintrag konnte gespeichert werden." }, { status: 500 });
  }
